...
version: "1.0.0"
description: |
  Use when enforcing premium frontend design quality by blocking generic AI-generated UI patterns.
  Controls design tone with 3 dials: VARIANCE, MOTION, DENSITY.
  Triggers: "디자인 맛", "UI 품질", "AI 냄새 제거", "프론트엔드 디자인", "taste", "리디자인"
---
...
* **INDUSTRY CONTEXT EXCEPTION:** When the product domain inherently requires blue (healthcare, fintech, B2B SaaS, enterprise software), blue IS permitted as the accent color. The ban targets gratuitous "AI purple/blue glow" aesthetics, not industry-appropriate blue usage.
...
When generating modern SaaS dashboards or feature sections, utilize the "Bento 2.0" architecture:
...
* Typography: `Geist`, `Satoshi`, or `Cabinet Grotesk`. Subtle `tracking-tight` for headers.
* Labels outside and below cards. Generous `p-8` or `p-10` padding inside.
...
* Infinite Loops: Every card has an active looping state (Pulse, Typewriter, Float, Carousel).
* Performance: `<AnimatePresence>`, `React.memo`, isolated Client Components for perpetual motion.

### C. 5-Card Archetypes
1. **Intelligent List** — auto-sorting with `layoutId`
2. **Command Input** — typewriter + processing shimmer
3. **Live Status** — breathing indicators + overshoot notifications
4. **Wide Data Stream** — infinite horizontal carousel
5. **Contextual UI (Focus Mode)** — staggered highlight + float-in toolbar

## 10. FINAL PRE-FLIGHT CHECK
- [ ] Global state not misused
...
When upgrading existing projects, follow this priority:
1. **Font swap** — Replace generic fonts first (highest visual impact)
2. **Color cleanup** — Remove AI purple/neon, apply neutral base + 1 accent
3. **Hover/Active states** — Add missing interactive feedback
4. **Layout refinement** — Break symmetry, remove generic 3-col cards
5. **Component upgrade** — Replace generic cards with spacing/elevation
6. **Loading states** — Add skeleton loaders
7. **Typography polish** — Tracking, line-height, max-width tuning

**Constraint:** Work with the existing tech stack. Do not migrate frameworks or styling libraries.

---

## 12. FULL OUTPUT ENFORCEMENT

* NEVER use `// ...`, `// rest of code`, `// TODO`, or `/* similar to above */`
* NEVER say "for brevity", "and so on", or "the rest follows the same pattern"
* When approaching token limits, pause at a clean breakpoint:
