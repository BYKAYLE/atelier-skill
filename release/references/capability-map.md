...
| pentest-router | 명시 승인된 침투/공격성 런타임 테스트 | 대상 URL/IP, 소유권, 승인 scope | Docker 필수, ~$50/회. 사용자 명시 승인 전 실행 금지 |
...
| private-deployment-skill | Docker→NAS/VPS 8Phase 배포 | 프로젝트 경로, 서버 정보 | Checkpoint 2회 |
| private-company-data-skill (v1.2.0) | 회사 정보+시장 인텔 DB | — | 수정/삭제 금지 |
| private-knowledge-wiki (v1.0.0) | Karpathy식 자동 지식 위키. Ingest/Query/Lint | — | raw/ 불변, wiki/ LLM 전용 |
...
| 스킬 | 역할 | 협조 방식 |
|------|------|----------|
| private-rd-orchestrator (v4.0.0) | 자율 심층 R&D (베가펑크+7연구원) | release 파이프라인에서 연구 필요 시 협조 요청. 상하 관계 아님 |
